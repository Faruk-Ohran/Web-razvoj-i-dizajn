...
$("#forma").validate({
    ...
    messages:{
        dostavaIme:{
            ...
            regex:"Unesite ime i prezime sa velikim pocetnim slovom"
        },
        dostavaAdresa:{
            required:"Ovo polje je obavezno",
            regex:"Samo slova"
...
function getPoziv(funk,url)
    ...
    document.getElementById("filtiranje1").oninput=function()
    {
        ocistiRedove();
        var unos=document.getElementById("filtiranje1").value;
        ...
        zahtjev.onload=function()
        {
            if(zahtjev.status==200)
            {
                ...
                {
                    if(json[i].naziv.startsWith(unos))
                    {
                        document.querySelector("#tabelaID tbody").innerHTML+=napraviRedove(json[i]);
                    }
                }
            }
        }
        zahtjev.open('GET',urlGetAllProizvodi,true);
        zahtjev.send();
...
/*var input=document.getElementById('naruci');
input.onsubmit=refresaj;
function refresaj()
{
    location.reload();
}*/
